...
| Klawisz | Jailor | Town Killing | Each night, choose a living player to jail. The jailed player is roleblocked for the night, the Jailor reads the jailed player's last will from their card, then the Jailor chooses EXECUTE (thumbs down) or SPARE (thumbs up). Execution is an Unstoppable kill. The Jailor cannot jail the same player on two consecutive nights. When the `jailorNoExecN1` house rule (off by default) is enabled, the Jailor cannot execute on Night 1; jailing and reading the will still work on Night 1. |
...
| Kucharz | Chef | Town Support | Start-knowing, no night action. You start knowing how many pairs of adjacent evil players there are in the seat circle. The count is relayed privately during the prep phase (Night Zero) and written to the Chef's player log as an info entry at SETUP. The Chef never wakes at night. The count is a snapshot of the deal: it never changes when roles are swapped. Evil is the Mafia-aligned bucket plus the Serial Killer: the Godfather, the Mafioso, a Witch who sides with Mafia, an Amnesiac who remembered a Mafia role, and the Serial Killer. The Demon, the Imp, and the other Evil-team roles (Succubus, Necromant, Possessed) are not counted. |
...
| Wiedźma | Witch | Neutral Evil | Each night, after the Poisoner, choose a living player to control plus a redirect target: the controlled player's targeting night action is redirected to that target. Cannot control a player the Jailor jails that night (the jail voids the control) or a dead player. On a successful control, the Witch learns the controlled player's exact role. Controlling the Godfather redirects the Mafia kill; controlling the Serial Killer or the Demon redirects their kill; controlling the Jailor redirects only the jail target, with the EXECUTE or SPARE decision staying with the Jailor; controlling the Witness redirects only its first pick. A control has no effect on a role without a redirectable targeting action (the Poisoner has already acted, role-blockers and revivers do not consume it), but the role is still revealed. `witchSide` is editable pre-game (seats screen, Night Zero checklist), default MAFIA: the Witch counts as Mafia-aligned and wins when Mafia wins, or as Town-aligned winning with Town if declared. |
| Szpieg | Spy | Neutral Benign | Each night, choose a living player and learn the team (Town / Mafia / Neutral / Evil) of every player who visited them that night; if nobody visited them, you learn "no one". If the Spy is Drunk, the learned teams are random. Wins if alive at game end (shares the win with whoever triggered the end). |
...
| Wyrzutek | Outcast | Neutral Benign | No ability. Reads as Evil to the Sheriff (SUSPICIOUS, per `_sheriffSuspicious`) and to the Oracle (NOT TOWN); for the Witness it lands in the Neutral triage bucket, and a Drunk Consigliere inspecting it is offered a false role from a different-alignment pool (Town, Mafia, or Evil). Wins if alive at game end. |
...
| Nekromanta | Necromant | Evil Support | Once per game, at night, choose a corpse whose power to borrow plus a living target, and use the corpse's night ability on that target; the power resolves through the corpse role's own night-step resolver (js/engine/06b-night-actions.js `borrowedResolverByRole` + `_nightActions`). Borrowable: Poisoner, Escort, Consort, Innkeeper, Doctor, Godfather, Mafioso, Janitor, Forger, Blackmailer, Demon, Serial Killer, Framer, Sheriff, Tracker, Lookout, Witness, Consigliere, Undertaker, Spy, Oracle, and Medium (a borrowed two-target power such as the Witness may carry its own second pick). Roles with no usable nightly power refuse the borrow and consume the once-per-game use: Civilian, Jester, Executioner, Leper, Outcast, Possessed, Survivor, Imp, Washerwoman, Chef, Vigilante, Deputy, Mayor, and The Drunk. Picking the Jailor, the Veteran, the Witch, the Succubus, the Necromant itself, the Retributionist, or the Amnesiac also fails (those powers are not borrower-usable) but does not consume the use. The Necromant is Evil-aligned for team counts and win conditions. |
| Morderca | Serial Killer | Neutral Killing | Each night, choose a living player to kill (Basic attack). Night immune: Basic defense blocks Basic attacks. Reads SUSPICIOUS to the Sheriff. Wins when last standing or holding majority. |
...
| Opętany | Possessed | Evil Support | Townsfolk disguise: no wake at night, no active ability. Reads SUSPICIOUS to the Sheriff and NOT TOWN to the Oracle; for the Witness it lands in the Neutral triage bucket. The role name stays hidden until end-of-game reveal. Counts as Evil for team counts and win conditions. |
...
- **Neutral**: benign roles that win only by surviving (Survivor, Spy, Leper, Amnesiac, The Drunk, Outcast) plus solo-evil neutrals that target the town by their own rules (Jester, Executioner) and the Witch (team `NEUTRAL`, victory by declared side).
- **Evil**: roles with `team: 'EVIL'` (Serial Killer is listed here but keeps `team: 'NEUTRAL'` in the engine — see below): Demon, Imp, Succubus, Necromant, Possessed, Serial Killer.

The engine tracks the Serial Killer with `team: 'NEUTRAL'`: it never counts toward the Evil side of the one-versus-one settlement; instead it is tracked by its own victory condition (`E._isSerialKiller`) and by the Town win gate (Town cannot win while the SK lives).

Evil roles count toward the Evil-side total in the one-versus-one deadlock resolution and in the Demon's own majority rule (see GDD §9.3). They are NOT aligned with the Mafia faction and do not share Mafia wins unless explicitly stated.
...
- Team values in the engine: `TOWN`, `MAFIA`, `NEUTRAL`, `EVIL`. The Witch remains `NEUTRAL` (Evil/Neutral Evil) but switches its alignment and victory condition by declared side (`witchSide`). The Serial Killer also keeps `team: 'NEUTRAL'` and is tracked by its own victory rule. Evil roles use `team: 'EVIL'`.
