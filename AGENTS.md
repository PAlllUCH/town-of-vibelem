...
- **Roles (35 total):** 30 base + Spy (Neutral Benign, shared win) + nightly-reading Oracle + start-knowing Washerwoman / Chef + nightly pair-comparison Witness (Town Invest.). No role is Night-1-only: the Oracle wakes every night; start-knowing roles deliver their info at the prep (Night Zero) phase via `state.playerLog` rows (kind `info`). Deck reach per preset: GDD §4.8.
...
- This project is built and maintained via the **orchestrator workflow**: plan → dispatch workers via `crush run "..." -m opencode-go/deepseek-v4-flash --cwd "<project-root>"` (default constraint: Flash workers only, no free tier; the owner may override with a MiMo designer for visual tasks) → mandatory review pass → fix loop → `node --test` until green.
