...
(function () {
  ...
  function card(title, body, key, app) {
    ...
    return '<div class="card card-collapsible' + (collapsed ? ' collapsed' : '') + '">' +
      ...
      '<button class="btn btn-sm btn-collapse" data-action="toggle-card" data-card="' + esc(key) + '"' +
      ' aria-expanded="' + (collapsed ? 'false' : 'true') + '" aria-controls="card-body-' + esc(key) + '">' +
      (collapsed ? '+' : '-') + '</button></div>' +
      '<div class="card-body" id="card-body-' + esc(key) + '">' + body + '</div></div>';
  }
